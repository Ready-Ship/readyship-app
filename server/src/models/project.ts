...
export class ProjectModel {
  ...
  async create(params: CreateProjectParams) {
    ...
    return this.client.query(
      'INSERT INTO project (creatorid, title) VALUES ($1, $2) RETURNING *',
      [creatorid, title]
    );
  }
  ...
  async findByUserId(userId: string) {
    const query =
      'SELECT * FROM project JOIN project_has_assignee AS pha ON project.id = pha.projectid where pha.userid = $1';
